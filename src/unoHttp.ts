import type { DiscoveredSystem, DiscoveredZone } from './unoProtocol.js';
...
//   "OPEN:  Garage Door"
export async function discoverSystem(host: string, password: string): Promise<DiscoveredSystem> {
  const url = `http://${host}/`;
  ...
  if (!response.ok) {
    if (response.status === 401) throw new Error('Authentication failed — check your password');
...
function parseHtml(html: string): DiscoveredSystem {
...
function parseZoneTitle(title: string, zoneNumber: number): DiscoveredZone | null {
  // Title format: "<STATE>: [<time-ago> ]<zone name>"
  const colonIdx = title.indexOf(':');
  if (colonIdx === -1) return null;

  const state = title.slice(0, colonIdx).trim().toUpperCase() as 'OPEN' | 'CLOSED';
...
function parsePartitionName(html: string): string {
  ...
  if (nameMatch) {
    ...
    if (text && !/ready|not ready|armed/i.test(text)) return text;
  }
  return 'Home';
}
